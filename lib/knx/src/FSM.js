  ...
  states: {
      ...
      _onEnter: function( ) {
        ...
        if (this.useTunneling) {
          ...
          setTimeout(() => {
            ...
            }.bind( this ), parseInt(process.env.KNX_CONNECT_RETRY_MS, 10) || 10000 );
            delete this.channel_id;
            delete this.conntime;
            delete this.lastSentTime;
            // send connect request directly
            this.send( sm.prepareDatagram( KnxConstants.SERVICE_TYPE.CONNECT_REQUEST ));
            // Delay between DISCONNECT_REQUEST and the following CONNECT_REQUEST
            // when re-establishing an existing channel. The stock 5 s wait means
            // that whenever a flaky router (zavod :3699 / new Dom) stops ACKing a
            // tunnelling request, the relay stays dead for >5 s before the tunnel
            // comes back — the visible "every other time" failure. A fresh tunnel
            // ACKs writes perfectly, so on those routers we reconnect almost
            // immediately (KNX_RECONNECT_DELAY_MS) and let the app-layer write
            // retry re-issue the command onto the healthy channel.
          }, this.channel_id ? (parseInt(process.env.KNX_RECONNECT_DELAY_MS, 10) || 5000) : 0)
        } else {
    ...
    requestingConnState: {
      _onEnter: function( ) {
        ...
        }.bind( this ), parseInt(process.env.KNX_CONNSTATE_INTERVAL_MS, 10) || 10000 );
      },
      _onExit: function() {
    ...
    sendDatagram:  {
      _onEnter: function ( datagram ) {
        ...
        this.send( datagram, function(err) {
          if (err) {
            // The previous code used `this.*` inside this nodejs-style
            // callback, but Connection.send doesn't bind `this` — so on the
            // error branch the FSM would crash with
            //   TypeError: this.transition is not a function
            // every time the IP gateway delayed/dropped a TUNNELING_ACK
            // (common on busy buses or just after (re)connect). The success
            // branch below correctly uses the captured `sm` reference; we
            // do the same here.
            if (sm.numberOfWaitingACKFORTunnellingReqAttempts < (parseInt(process.env.KNX_ACK_RETRIES, 10) || 3)) {
              sm.numberOfWaitingACKFORTunnellingReqAttempts++;
              sm.transition( 'sendDatagram', datagram );
            } else {
              sm.numberOfWaitingACKFORTunnellingReqAttempts = 0;
              sm.transition( 'connecting' );
            }
          } else {
      ...
      // See sendTunnReq_waitACK: ACK inbound bus telegrams inline instead of
      // deferring, otherwise a busy router deadlocks the tunnel while we are
      // mid-send and forces a reconnect on nearly every write.
      'inbound_TUNNELING_REQUEST_L_Data.ind': function ( datagram ) {
    ...
    sendTunnReq_waitACK:  {
      _onEnter: function ( datagram ) {
        ...
        this.tunnelingAckTimer = setTimeout( function() {
          ...
          if(this.numberOfWaitingACKFORTunnellingReqAttempts < (parseInt(process.env.KNX_ACK_RETRIES, 10) || 3)){
        ...
        }.bind( this ), parseInt(process.env.KNX_ACK_TIMEOUT_MS, 10) || 1000 );
      },
      _onExit: function () {
      ...
      // Acknowledge inbound bus telegrams even while we are still waiting for
      // the router's ACK of OUR outbound request. Per the KNXnet/IP tunnelling
      // spec every inbound TUNNELING_REQUEST must be ACKed immediately, and
      // many routers (notably busy buses like the zavod site, where a device
      // continuously spams GroupValue_Responses) will NOT ACK our outbound
      // request until we have ACKed their inbound one. The old behaviour
      // deferred these via the "*" handler, which deadlocks the tunnel: we
      // wait for their ACK, they wait for ours, we time out after 1s, retry 3x
      // and reconnect. The relay then appears to switch only "every other
      // time". ACK inline and stay in this state so our own ACK wait survives.
      'inbound_TUNNELING_REQUEST_L_Data.ind': function ( datagram ) {
        this.seqnumRecv = datagram.tunnstate.seqnum;
        this.acknowledge(datagram);
        this.emitEvent(datagram);
